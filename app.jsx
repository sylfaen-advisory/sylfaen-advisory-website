        ...
        <div className="practice-grid" style={!showEtymology ? { gridTemplateColumns: '1fr' } : {}}>
          {showEtymology && <aside className="practice-aside">
            ...
            <figure style={{ margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div className="is-portrait" style={{ border: '1px solid var(--rule)', overflow: 'hidden', background: '#1a1612' }}>
                <img
                  src="assets/divider-desk.jpg"
                  alt="A pencil sketch of a building elevation on a wooden desk"
                  style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: '20% 80%', display: 'block', filter: 'saturate(0.75) contrast(1.05) brightness(0.92)' }}
                />
              </div>
              <figcaption style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--slate)' }}>
                <span>Pl. II · At the desk</span>
                <span>Where engagements begin</span>
              </figcaption>
            </figure>
          </aside>}
    ...
    try {
      const res = await fetch("https://formspree.io/f/mqejaqpg", {
        ...
        body: JSON.stringify({
          ...
          organisation: org,
          email,
          interested_in: chosen,
          context: msg,
          reference: refId,
          _subject: `Sylfaen enquiry · ${chosen} · ${name}`,
        }),
      });
      if (!res.ok) {
      ...
      setError(err.message || "Something went wrong — please email mark@sylfaenadvisory.co.uk directly.");
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <div className="contact-aside">
            ...
            <div className="principal-card">
              <div className="principal-photo">
                <img src="assets/headshot-mark.jpg" alt="Mark Biscoe, Principal of Sylfaen Advisory" />
              </div>
              <div className="principal-meta">
                <span className="principal-eyebrow">You'll speak with</span>
            ...
            <div className="contact-modes">
              ...
              <a href="https://calendly.com/mark-sylfaenadvisory/30min" target="_blank" rel="noopener noreferrer" className="contact-mode">
              ...
              <div className="form-foot">
                <span className="note">
                  {error
                    ...
                    : "Held in confidence by default · NDAs available on request."}
                </span>
...
// === Divider plate (narrow editorial band) ===

function Divider({ src, roman, label, caption, marker, height = "260px" }) {
  return (
    <section className="plate plate-divider" aria-label={`${roman} — divider`}>
      <div className="plate-frame" style={{ height }}>
        <img src={src} alt="" />
        <div className="plate-top-rule">
          <span>{roman}</span>
          <span>{marker}</span>
        </div>
        <div className="plate-caption plate-caption-tight">
          <div className="plate-label" style={{ fontSize: 'clamp(16px, 1.6vw, 22px)' }}>
...
function PlateIII() {
  return (
    <section className="plate" aria-label="Plate III — On site">
      <div className="plate-frame" style={{ height: 'clamp(360px, 56vh, 620px)' }}>
        <img src="assets/plate-landscape.jpg" alt="Pen y Fan, Brecon Beacons, Wales — graded to match the brand palette" />
        <div className="plate-top-rule">
          <span>Sylfaen Advisory · Figure plate</span>
          <span>Pen y Fan · Bannau Brycheiniog</span>
        </div>
        <div className="plate-caption">
          <div className="plate-label">
            <span className="roman">Plate III</span>
            From the foundation up — <em>where the practice begins.</em>
          </div>
          <div className="plate-meta">
            <span>Sylfaen, n. (Welsh)</span>
            <span className="v">Foundation · base · ground</span>
          </div>
...
function App() {
  ...
  return (
    <>
      ...
      <Divider
        src="assets/divider-precision.jpg"
        roman="Plate II ½ · Method"
        label="Plate II ½"
        caption={<>The work is measured — <em>fixed scope, fixed fee, fixed deadline.</em></>}
        marker="On precision · Engagements →"
      />
      <Engagements />
      <AwenSection />
      <PlateIII />
